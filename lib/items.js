...
function get_matching_field (items, field, value) {
...
// root -done
// field match (check if done, etc)
// field search (look in body, title, etc)
...
function AND_query_handler (user, matches, queries) {
  ...
  for (const query of queries) {
    switch (query.type) {
      ...
      case 'match':
        matches = AND_arrays(matches, get_matching_field(user.items, query.field, query.value))
      break;
    }
  }
...
function OR_query_handler (user, matches, queries) {
  ...
  for (const query of queries) {
    switch (query.type) {
      ...
      case 'match':
        matches.push(...get_matching_field(user.items, query.field, query.value))
      break;
    }
  }
